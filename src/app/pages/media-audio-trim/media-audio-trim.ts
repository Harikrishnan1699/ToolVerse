...
@Component({
  selector: 'app-media-audio-trim',
  imports: [Dropzone, FormsModule, SectionHeader],
  template: `
    <app-section-header title="Audio Trimmer" subtitle="Cut a section from an audio file — preserves the original codec." icon="✂" color="from-cyan-500 to-blue-600" back="/media" backLabel="All media tools" />
    <section class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 space-y-5">
      @if (!file()) {
        <app-dropzone title="Drop an audio file" accept="audio/*" (files)="pick($event)" />
      } @else {
        <div class="card p-6 space-y-5">
          <audio [src]="url()" class="w-full" controls></audio>
...
export class MediaAudioTrim {
  ...
  async run() {
    ...
    try {
      ...
      const ext = this.file()!.name.match(/\.[^.]+$/)?.[0] ?? '.mp3';
      await this.ffmpeg.writeFile('in' + ext, this.file()!);
      const dur = (+this.end - +this.start).toFixed(2);
      await this.ffmpeg.exec(['-ss', String(this.start), '-i', 'in' + ext, '-t', dur, '-c', 'copy', 'out' + ext]);
      const data = await this.ffmpeg.readFile('out' + ext);
      const blob = new Blob([data as BlobPart], { type: this.file()!.type || 'audio/mpeg' });
      saveAs(blob, (this.file()?.name ?? 'audio').replace(/\.[^.]+$/, '') + '-trimmed' + ext);
